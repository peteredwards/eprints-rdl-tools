...
my %config = (
	...
	filename => {
		# allow spaces
		allow_spaces => 0,
		# allow "other" chars (i.e. those not A-Z, a-z, 0-9, "_", ".", "-", " ")
		allow_others => 0,
		# uri_escape all necessary filename characters
		uri_escape => 0,
	},
);
...
my %schema = (
	...
	researchdata => {
		...
		'creators.name^family' => 'M',
		'creators.name^given' => 'M',
		'creators.affiliation1' => 'M',
		'creators.affiliation1_id' => 'M',
		'creators.affiliation1_type' => 'M',
		'creators.affiliation2' => 'M',
		'creators.affiliation2_id' => 'M',
		'creators.affiliation2_type' => 'M',
		'corp_creators' => 'M',
		'data_type' => '',
		'contributors.id' => 'M',
		...
		'contributors.name^given' => 'M',
		'contributors.type' => 'M',
		'contributors.affiliation1' => 'M',
		'contributors.affiliation1_id' => 'M',
		'contributors.affiliation1_type' => 'M',
		'contributors.affiliation2' => 'M',
		'contributors.affiliation2_id' => 'M',
		'contributors.affiliation2_type' => 'M',
		'funders' => 'M',
		'grant' => 'M',
		'projects' => 'M',
		'collection_date.from' => 'M',
		...
		'documents.doi' => '',
		'documents.note' => '',
		'documents.dpformat.description' => '',
		'documents.dpformat.tool' => '',
		'documents.format_registry.format_name' => '',
		'documents.format_registry.format_version' => '',
		'documents.format_registry.format_id' => '',
		'documents.format_registry.registry_name' => '',
		'documents.equipment' => '',	
		'documents.software.name' => '',
		'documents.software.version' => '',
		'documents.encryption.name' => '',
		'documents.encryption.version' => '',
		'documents.main' => '',
		'documents.path' => 'M',
	},
...
if ( @filename_spaces || @filename_others ) {
	my $err_str;
	if ( @filename_spaces ) {
		$err_str .= "ERROR: The following " . scalar @filename_spaces . " file references contain spaces\n";
		foreach ( @filename_spaces ) {
			$err_str .= "$_\n";
		}
		$err_str .= "\n";
	}
	if ( @filename_others ) {
		$err_str .= "ERROR: The following " . scalar @filename_others . " file references contain disallowed characters\n";
		foreach ( @filename_others ) {
			$err_str .= "$_\n";
		}
		$err_str .= "\n";
	}
				...
				foreach $field (keys %{$eprints{$eprintid}{documents}{$docid}}) {
					...
					if ($field eq $config{docpath}{field}) {
						...
						foreach $value (@{$eprints{$eprintid}{documents}{$docid}{$field}}) {
							...
							# construct the full filepath bit by bit starting with the pre-path
							my $fullfilepath = $config{docpath}{prefix};
							# remove any file:// prefix from the pre-path
							$fullfilepath =~ s/^file\:\/\///i;
							# do the same for the filepath value
							$value =~ s/^file\:\/\///i;
							# add the filepath value onto the full filepath
							$fullfilepath .= $value;
							# check for disallowed characters...
							# spaces
							if ( !$config{filename}{allow_spaces} && ( XML_unescape($fullfilepath) =~ m! ! )) {
								push @filename_spaces, XML_unescape($fullfilepath);
							}
							# "other" characters (other than A-Z, a-z, 0-9, "_", ".", "-", " ")
							if ( !$config{filename}{allow_others} && ( XML_unescape($fullfilepath) =~ m![^_A-Za-z0-9\-\.\/ ]! )) {
								push @filename_others, XML_unescape($fullfilepath);
							}							
							# URI escape the full filepath if required
							$fullfilepath = uri_safe($fullfilepath) if $config{filename}{uri_escape};
							# output the full filepath prefixed with file:// and URI encoded
							print OP "\t\t\t\t\t\t\t<url>file://".$fullfilepath."</url>\n";
							print OP "\t\t\t\t\t\t</file>\n";
						}
						print OP "\t\t\t\t\t</files>\n";
						next;
...
sub show_usage {
	...
	print "    -f = allow spaces in file references (filenames and paths)\n";
	print "    -F = allow 'other' characters in file references (filenames and paths)\n";
	print "    -c = URI-escape file references (no effect unless -f or -F active)\n";
	print "    -t <eprint_status> = set eprint_status to <eprint_status> (default:buffer)\n";
	print "    -v = display version number\n\n";
	print "  Notes:\n";
	print "    - 'other' characters are those not A-Z, a-z, 0-9, '_', '.', '-', ' '\n";
	print "    - to allow spaces AND other characters use both -f and -F options\n";
	print "    - currently eprints does NOT handle 'other' filename characters properly\n";
...
sub get_options {
	...
	GetOptions( 'help|?' => sub { show_usage();},
				...
				'fnspaces|f' => \$config{filename}{allow_spaces},
				'fnothers|F' => \$config{filename}{allow_others},
				'fnescape|c' => \$config{filename}{uri_escape},
				);
...
# unescape unsafe XMl characters (to revert to non-XML-safe string)
sub XML_unescape
{
	my $s = shift;
	$s =~ s/&lt;/\</g;
...
# return a URI safe version of the string (encode non-ASCII chars)
sub uri_safe
{
	my $s = shift;

	# first we need to unescape the XML encoding
	$s = XML_unescape($s);
	# next we URI encode the string (assumes UTF8)
	$s = uri_escape_utf8($s);
	# lastly we put the forward slashes back in place
	$s =~ s/%2F/\//g;
	
	return $s;
}

# checks for disallowed filename characters
sub filename_chars
{
